...
def make_preview() -> None:
    ...
    # Soft split background inspired by the approved homepage concept.
    for x in range(W):
        if x < 720:
            t = x / 720
            c1 = (247, 250, 247)
    ...
    # Right-side lifestyle illustration: folded towels, bottle and plant.
    # Plant stems/leaves.
...
def inject_meta() -> None:
    for path in SITE_DIR.rglob("*.html"):
        ...
        canonical = extract(r'<link\s+rel="canonical"\s+href="([^"]*)"', text, "https://homekeepinglab.com/")
...
if __name__ == "__main__":
    make_preview()
    inject_meta()
    print(f"Wrote {OUT_PATH} and injected Open Graph/Twitter metadata.")
